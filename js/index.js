...
//Block colors
const colors = ["34495e","E74C3C","9b59b6","2980b9","f1c40f","2ecc71","3498db","e67e22"];
...
//stores number of genomes, reccomended ~50 
...
function initialize(){
  ...
  loadArchive(localStorage.getItem("archive") || null);
  updateHistory();
  updateTopScores();
	//the game loop
	var loop = function(){
...
window.onkeydown = function (event){
    ...
    */
    ...
    //*/
		...
		ai = !ai;
    //*/
  /* DISABLED FOR NOW AS WAS GETTING IN THE WAY OF CTRL+R RELOADING
 ...
 */
 ...
 function updateHistory(){
   let html = '';
   clone(archive.elites || []).forEach((spawn, index)=>{
     html = `<tr><td>${(index + '').padStart(3,0)}</td><td>${spawn.fitness}</td><td>${spawn.moves}</td></tr>${html}`;
   });
   document.getElementById("history-content").innerHTML = html;
 }
 function showTopScores(){
   document.getElementById("topscores").style.display = 'table';
   document.getElementById("history").style.display = 'none';
 }
 function updateTopScores(){
   let html = '';
   clone(archive.topscores || []).some((spawn, index)=>{
     if (index >= 10) return true;
     html += `<tr><td>${spawn.id}</td><td>${spawn.generation}</td><td>${spawn.fitness}</td><td>${spawn.moves}</td></tr>`;
   });
   document.getElementById("topscores-content").innerHTML = html;
 }
 ...
 */
 function evolve() {
  ...
  document.getElementById("Generation").innerHTML = generation;
 	...
 	console.log("Elite's fitness:", genomes[0].fitness, "\nMoves Taken:", genomes[0].moves);
  // check if elites have a higher score than previous high scores
  clone(genomes).some((spawn, index)=>{
    if (archive.topscores.length == 10 && archive.topscores[9].fitness >= genomes[0].fitness) return true;
    if (archive.topscores.length < 10)
      archive.topscores.push(spawn);
    else
      archive.topscores[9] = spawn;
    archive.topscores.sort(function(a, b) {
      return b.fitness - a.fitness;
    });
  });
 ...
 */
 function makeNextMove() {
 	...
 	if (movesTaken > moveLimit) {
 		...
 		genomes[currentGenome].moves = clone(movesTaken);
 ...
 function output() {
 	if (draw) {
 		...
 		for (var c = 0; c <= colors.length; c++) {
 			html = replaceAll(html, c + ",", "<font color=\"" + colors[c] + "\">◼</font>,");
 			html = replaceAll(html, "," + c, "<font color=\"" + colors[c] + "\">◼</font>");
 		}
 ...
 function updateNextShape() {
 	if (draw) {
 		...
 		for (var i = 0; i < upcomingShape.length; i++) {
 			var next =replaceAll((upcomingShape[i] + ""), "0", "<font style='opacity: 0;'>◼</font>");
 			html += "<br />" + next;
 		}
 		...
 		for (var c = 0; c < colors.length; c++) {
 			html = replaceAll(html, "," + (c + 1), ",<font color=\"" + colors[c + 1] + "\">◼</font>");
 			html = replaceAll(html, (c + 1) + ",", "<font color=\"" + colors[c + 1] + "\">◼</font>,");
 		}
 		html = replaceAll(replaceAll(replaceAll(html, "&nbsp;,", "&nbsp;&nbsp;"), ",&nbsp;", "&nbsp;&nbsp;"), ">,<", ">&nbsp;<");
 		document.getElementById("next_shape").innerHTML = html;
 	}
  return;
}

 function updateNextShae() {
 	if (draw) {
 		html = "<br />Speed: " + speed;
 		if (ai) {
 		...
 		document.getElementById("details_other").innerHTML = html;
 	}
 }
